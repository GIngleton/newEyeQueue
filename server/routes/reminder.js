...
var reminder = {
  ...
  getAll: function(req, res) {
    ...
    if(!reminder.userId) {
      ...
      res.json({
        "status": 400,
        "message": msg_reminder_userIDMissing
      });

    } else {
      try {
        var reminder = require('../logic/reminderreminder').getAll(reminder);
        res.status(200);
        res,json({
          "status": 200,
  ...
  create: function(req, res) {
    ...
    if (!reminder.remindThis || !reminder.shdlCall || !reminder.shdlSMS) {
      ...
      if (!_v.validateSchdl(reminder.schdlCall) || !_v.validateSchdl(reminder.shdlSMS)) {
        res.status(400);
        res.json({
          "status": 400,
      ...
      if (user) {
        ...
        res.json({
          "status": 200,
          "message": msg.reminder_nwReminderSuccess,
